...
async function run() {
  const query = process.env.BING_QUERY || '你好世界';
  ...
  try {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      finalState = await evaluate(
        ...
        sessionId,
        `(() => ({
          inputText: document.querySelector('#tta_input_ta')?.innerText || '',
          outputText: document.querySelector('#tta_output_ta')?.innerText || '',
          outputHTML: document.querySelector('#tta_output_ta')?.innerHTML || '',
          extractedText: (document.querySelector('#tta_output_ta')?.textContent || '').trim(),
          outputLang: document.querySelector('#tta_output_ta')?.getAttribute('lang') || ''
        }))()`
      );
      const output = (finalState.outputText || '').trim();
      if (output && output !== '...') {
        break;
      }
    }
    ...
    console.log(JSON.stringify({
      ...
      injected,
      finalState,
      success: /hello/i.test(finalState?.outputText || '')
    }, null, 2));
  } finally {
