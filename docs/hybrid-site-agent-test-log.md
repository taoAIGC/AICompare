# Hybrid Site + Agent Test Log

Date: 2026-05-16

## Scope

This file records implementation verification for the hybrid site + agent compare feature.
...
- [x] sites and agents render together
...
- [x] local agent submit only affects that agent
...
- [x] error state isolated to failed agent panel
...
- [x] agent full read-only conversation visible
...
- `settings-agent-utils-missing-script`: `options/options.html` initially missed `shared/agent-prompt-utils.js`, which broke default agent engine normalization.
...
- `hybrid-history-open-params`: hybrid history cards incorrectly passed synthetic `Agent:xxx` labels through `sites=`; now hybrid detail relies on `historyId` recovery only.
- `agent-category-batch-select-gap`: homepage lacked category-level multi-select; now double-clicking a category chip toggles all agents in that category.
- `agent-panel-close-refresh-not-cancelling`: closing or refreshing an agent panel could leave the old backend job running; panel lifecycle now sends explicit cancel messages.
- `initial-agent-query-not-running`: opening compare with `query=...&agents=...` created agent panels but did not send the first question automatically; now initial query fans out to selected agents too.
...
- `live-hybrid-copy-including-agent-iframes-as-sites`: timeline copy collection previously treated all `.ai-iframe` nodes as site iframes. Live collection now uses site iframes for DOM extraction and agent panel state for agent answers.
...
- `agent-engine-config-read-from-redacted-background-response`: the live iframe-side agent runner briefly tried to read engine config through a background message that intentionally omitted `apiKey`, so every agent request failed with `Agent engine is not configured` despite a saved key. The iframe runner now reads the full config directly from Chrome storage before calling the API.
